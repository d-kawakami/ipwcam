...
このアプリケーションは**ローカルエリアネットワーク（LAN）内での使用のみ**を想定しています。認証や暗号化は実装していません。インターネットや信頼できないネットワークにサーバーを公開しないでください。

### Flask デバッグモードについて

サーバーは `debug=True` で起動します（`app.py` 298 行目）：

```python
app.run(host="0.0.0.0", debug=True, port=5500, threaded=True)
```

開発時に便利な Werkzeug のインタラクティブデバッガおよび自動リロードが有効になりますが、**本番環境での使用は推奨しません**。共有環境や本番設定で動かす場合は `debug=True` を `debug=False` に変更してください。
